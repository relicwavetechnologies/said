...
pub fn spawn() -> Result<BackendHandle, String> {
    ...
    let child = Command::new(&bin)
        ...
        .env("GATEWAY_API_KEY",  std::env::var("GATEWAY_API_KEY").unwrap_or_default())
        .env("DEEPGRAM_API_KEY", std::env::var("DEEPGRAM_API_KEY").unwrap_or_default())
    ...
    let endpoint = BackendEndpoint { url: url.clone(), secret };
...
fn find_binary() -> Result<PathBuf, String> {
    ...
    candidates
        ...
        .find(|p| p.exists())
        .ok_or_else(|| {
            "polish-backend binary not found — run `cargo build -p polish-backend --release` first".into()
        })
}
