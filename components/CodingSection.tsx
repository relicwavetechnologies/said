...
export function CodingSection() {
  ...
  useMotionValueEvent(scrollYProgress, 'change', (p) => {
    const clamped = Math.max(0, Math.min(SLIDES.length - 1, Math.floor(p * SLIDES.length)));
    setActive(clamped);
  });
  ...
  return (
    ...
    >
      <div className="sticky top-0 flex h-screen flex-col overflow-hidden">
        <BgGlow />
        <div className="relative mx-auto flex h-full min-h-0 w-full max-w-[1280px] flex-col px-token-md pt-6 pb-5 md:pt-8 md:pb-6">
          <motion.div
            className="max-w-[760px] shrink-0"
            initial={{ opacity: 0, y: 24 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.3 }}
            transition={{ duration: 0.7, ease: EASE_OUT }}
          >
            <p className="text-[12.5px] uppercase tracking-[0.2em] text-white/40">
              Coding &amp; Prompting
            </p>
            <h2 className="mt-2 text-[32px] font-normal leading-[1.04] tracking-[-0.02em] sm:text-[40px] md:text-[46px] lg:text-[50px]">
              Prompt faster with your <span className="font-display text-accent">voice</span>
            </h2>
          ...
          <div className="mt-4 grid shrink-0 grid-cols-1 items-start gap-y-3 md:grid-cols-12 md:gap-x-8">
            <AnimatePresence mode="wait">
              ...
              >
                <h3 className="text-[16px] font-medium tracking-[-0.005em]">{slide.title}</h3>
...
function BgGlow() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      <motion.div
        className="absolute -left-40 top-1/3 h-[420px] w-[420px] rounded-full bg-[radial-gradient(circle_at_center,rgba(103,190,255,0.18),transparent_60%)] blur-2xl"
        animate={{ x: [0, 40, -20, 0], y: [0, -20, 30, 0] }}
        transition={{ duration: 22, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute -right-40 bottom-1/4 h-[420px] w-[420px] rounded-full bg-[radial-gradient(circle_at_center,rgba(124,58,237,0.16),transparent_60%)] blur-2xl"
